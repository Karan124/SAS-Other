...
Possible processes guidance: When evidence is ambiguous or thin, still
consider whether the process is a POSSIBLE payment process based on
available signals (channel, customer segment, named systems, payment
action verbs). Do not invent facts or infer relationships that are not
supported by the supplied evidence. If genuinely no payment signal exists
at all, mark is_payment_process=false. The goal is to surface possible
payment processes for human review, not to force a confident answer either
way.

Be open-minded about scope. Payment-related processes include not only
direct payment execution but also ITGC-type processes, change management,
access management, monitoring, and governance processes WHERE they are
explicitly scoped to payment systems, payment platforms, or payment
transaction processing. Do not assume a process is excluded just because
its category sounds like "IT" or "governance" — assess the actual evidence.

──────────────────────────────────────────────────────────────────────────────
STEP 0 — SCOPE CHECK  (lightweight filter — apply with an open mind)
──────────────────────────────────────────────────────────────────────────────

Mark is_payment_process=false and set exclusion_gate_applied=true ONLY where
the process has NO plausible connection to payment systems, payment
instructions, payment processing, payment governance, or payment-scoped
technology/operations, AND the process is clearly one of:
  ...
  • Pure marketing or customer communications unrelated to payment execution
  • Pure HR/workforce activity that does NOT involve paying anyone
    (e.g. training, performance reviews, recruitment) —
    NOTE: payroll, staff payment runs, and employee disbursement processes
    are NOT excluded here — they are a valid payment category
    (Supplier / Contractor / Employee Payments) and must proceed to Step 1.
  • Pure procurement administration with no payment execution link
    (e.g. vendor selection, contract negotiation) — NOTE: actual vendor
    PAYMENT processing is not excluded here.
...
Do NOT exclude IT, ITGC, change management, access management, monitoring,
governance, business continuity, or third-party oversight processes at this
gate. These process types frequently ARE genuine payment processes (e.g.
access provisioning to SWIFT, monitoring NPP transaction logs, change
management for BPAY platform, vulnerability management for payment
infrastructure). Let Step 1 evaluate these on their evidence.

If there is any plausible payment signal — a named payment system, a
payment action, a payer-payee hint, a payment-relevant channel, or a
payment-relevant customer segment — do not exclude. Proceed to Step 1.

──────────────────────────────────────────────────────────────────────────────
STEP 1 — PAYMENT RELEVANCE ASSESSMENT
──────────────────────────────────────────────────────────────────────────────

Determine whether the process is a payments process or payments-enabling
process based only on the supplied evidence.

A process is payment-related if its function is one or more of:
  ...
  2. System or technology support
     ...
     monitoring, infrastructure, and access — where tied to payment
     execution, control, or payment system governance.

  3. Third-party involvement
     Involves external service providers participating in payment
     processing or payment scheme operation.

  4. Governance and oversight
     Provides control, oversight, or management required to execute,
     control, or oversee payments — including change management,
     resilience, business continuity, crisis management, third-party
     governance, risk oversight, access management, and incident
     oversight — where tied to payments systems or payments operations.

  5. Internal payment disbursement
     Pays the bank's own employees, contractors, or suppliers (payroll,
     vendor payments, contractor invoices).

Evidence required for is_payment_process=true (at least one of):
  (a) Explicit payer-payee relationship
  (b) Explicit payment action — e.g. settle, disburse, process payment, pay
  (c) Explicit payment system/scheme/network — e.g. NPP, RTGS, BPAY, SWIFT,
      RITS, PEXA, Direct Entry, Visa, Mastercard, eftpos
  (d) Explicit system, third-party, access, or governance reference in the
      context of payment processing — e.g. access provisioning for SWIFT,
      change management for a payment platform, monitoring payment logs
  (e) Reference to paying or disbursing funds to employees, contractors,
      or suppliers (payroll, vendor payment runs)

If none of (a)-(e) is present and Step 0 did not already exclude: assess
based on the strongest available signal. If no payment signal exists at
all, is_payment_process=false.

Confidence for payment_process_confidence:
  High   — Named payment system/scheme AND explicit payment action,
            payer-payee relationship, or internal disbursement reference.
  ...
  Low    — Payment relevance possible but indirect, thin, or requires
            inference. Flag for human review. This is acceptable —
            do not force a higher confidence than the evidence supports.
...
Process must create, control, or act upon a payment instruction, or be
directly tied to technology/governance that materially influences payment
execution.
  ...
  Exclude: no interaction with payment instruction or payment system at all;
           pure monitoring/reporting on non-payment systems; indirect process
           with no direct effect on payment movement or payment system.
...
- Payroll, staff payment, vendor payment, and contractor payment processes are
  NEVER excluded at Step 0 - they map to Supplier / Contractor / Employee
  Payments.
- ITGC, change management, access management, and governance processes are
  NEVER excluded at Step 0 solely for being IT/governance in nature - assess
  on payment system evidence.
...
- Naming alone must never solely drive a mapping decision (R6 supports only).
- Channel and customer segment fields must corroborate, not solely drive,
  a mapping - always combine with R1-R3 evidence.
- Where key fields are missing, state this and be conservative about
  confidence level, not about whether to consider the process.
- Do not invent facts. Do not infer relationships not supported by evidence.
"""
...
def build_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = [
        ...
        ("Channel/segment used in mapping", int(channel_seg_used_count)),
        ("Channel/segment claim hallucinated", int(channel_seg_hallucinated)),
        ("──────────────────────────", "──────────────"),
