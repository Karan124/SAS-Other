...
MANDATORY FOR DIRECT PROCESSES: Assign exactly one primary_category.
...
──────────────────────────────────────────────────────────────────────────────
USING CHANNEL AND CUSTOMER SEGMENT AS PAYER-PAYEE EVIDENCE (Direct only)
──────────────────────────────────────────────────────────────────────────────

Each process may include two additional fields:
...
How to use (Direct processes only):
  - "Institutional" segment + settlement/clearing context → Institution to Institution
  - "Consumer" or "Commercial" + deposit/repayment/BPAY  → Customer to Institution
  - "Consumer" + disbursement/refund/interest             → Institution to Customer
  - "Consumer"/"Commercial"/"Small Business" + P2P/card   → Customer to Customer
  - Channel alone does not determine category — combine with segment and action.
...
Deterministic signal priority:
  ...
  - Payroll / vendor / supplier outbound       → Supplier / Contractor / Employee
...
- If payment_process_type = "Direct":
    primary_category MUST be assigned. Use closest alternative at Low confidence
    if no direct evidence. Do not return null.
