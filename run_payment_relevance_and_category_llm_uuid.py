...
This version adds:
  - Enabling processes: classified as payment-related but NO category assigned
  - Checkpoint / resume: safe to interrupt and restart — picks up from where it stopped
  - Incremental JSONL writing: each batch saved to disk immediately
  - Progress display: batch count, % complete, ETA, running cost (AUD)
  - Cost tracking: running input/output token totals and AUD estimate per batch
  - --force flag: ignore checkpoint and reprocess from scratch
  - INTER_BATCH_SLEEP: small delay between batches to avoid rate limiting
...
ENABLING — is_payment_process=true, payment_process_type="Enabling"
  ...
  Enabling processes do NOT proceed to category mapping.
  Set mapped_categories=[] and primary_category=null for ALL Enabling
  processes. Do not attempt to assign a payment category to Enabling
  processes even if segment, channel, or L2 context suggests one.
...
MANDATORY FOR DIRECT PROCESSES ONLY: Assign exactly one primary_category.
Do not return null primary_category or empty mapped_categories for any Direct
process. If no direct evidence exists, assign the closest alternative category
with category_confidence_score=Low and explain the inference basis.

DO NOT ASSIGN CATEGORY FOR ENABLING PROCESSES: If payment_process_type is
"Enabling", set mapped_categories=[] and primary_category=null. Enabling
processes support payment infrastructure but do not directly execute a
specific payer-payee payment flow. Category assignment is not applicable.
Do not use segment, channel, or L2 context to infer a category for Enabling
processes — leave it blank regardless of any available signals.

If direct evidence does not support a category (Direct processes only):
...
If payment-related (Enabling):
  Never assign a category. Set mapped_categories=[] and primary_category=null.
  This is correct behaviour — do not flag this as missing.
...
- If payment_process_type="Direct": primary_category MUST be assigned.
  Use closest alternative with Low confidence if needed. Do not return null.
- If payment_process_type="Enabling": mapped_categories=[] and
  primary_category=null. Do NOT assign a category under any circumstances.
- Do not return empty categories for Direct processes if a closest
  alternative exists.
...
def validate_output(parsed: dict, batch: list) -> list:
    ...
    for m in mappings:
        ...
        # Enabling processes intentionally have null primary_category
        if m.get("is_payment_process") and ptype == "Direct" and not primary:
            warnings.append(
                f"Direct process has null primary_category for UUID {uid}. "
                f"Check closest alternative was applied."
            )
        if ptype == "Enabling" and primary:
            warnings.append(
                f"Enabling process has unexpected primary_category '{primary}' "
                f"for UUID {uid}. Category should be null for Enabling."
            )
